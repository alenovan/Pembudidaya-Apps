...
class LoginBloc {
  ...
  Future feedSR(_pondid, _fishdead, _created_at) async {
    ...
    try {
      ...
      String rawJson = '{"message":"Silahkan ulangi kembali"}';
      return jsonDecode(rawJson);
    }
  }
  ...
  Future<dynamic> analyticsMonitorByDate(String pond_id, String date) async {
    var pakan = await _repository.analyticsMonitorByDate(pond_id, date);
    var enc = json.decode(pakan.body)['data'];
    return enc;
  }

  Future<List<String>> analyticsCalendar(
      String pond_id, String from, String to) async {
